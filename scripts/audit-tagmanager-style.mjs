...
const targets = [
  path.join(root, 'src', 'components', 'TagManagerModal.tsx'),
  path.join(root, 'src', 'components', 'TagManagerModalParts'),
];
const exts = new Set(['.tsx', '.css']);

const allowedExact = new Set(['custom-scrollbar']);
const allowedPrefix = ['tag-manager-'];
...
function checkToken(token) {
  ...
  if (/^[a-zA-Z][a-zA-Z0-9_-]*$/.test(token)) {
    return `非 TagManager 前缀类名: ${token}`;
  }
...
async function main() {
  ...
  if (issues.length === 0) {
    console.log('✅ TagManager 样式审计通过');
    return;
  }

  console.error(`❌ TagManager 样式审计失败，共 ${issues.length} 项`);
